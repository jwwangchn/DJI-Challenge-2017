...
#define YSPEED 1300 //forward speed
#define XSPEED 600
#define ZSPEED 1300
#define ZSPEED_VISION 400
#define GRASPSPEED 2000
#define LRSPEED 100
#define LRDISTANCE 100
#define GRASPUP 80

#define ZROTATION90DEG 1572
#define CLAW_CLOSE_SONAR_TRIGGER_DISTANCE 27

#define DIFFCONST 50

#define BACKWARD_DISTANCE 300
using namespace cv;
using namespace std;
using cmt::CMT;
...
int main(int argc, char **argv)
{
    ...
    while ((!exit_flag)) //&&(capture.read(frame)))
    {
        ...
        if ((txKylinMsg.cbus.fs & (1u << 30)) == 0x00000000)
        {
            ...
            if (workState == 4 && sr04maf[SR04_IDX_F].avg > BACKWARD_DISTANCE)
            {
            ...
            {
                if (sr04maf[SR04_IDX_F].avg > BACKWARD_DISTANCE && coutLogicFlag_PutBox2toBox1 == 10.1)
                {
        ...
        case 3:
            ...
            if (finishAbsoluteMoveFlag_Put == false)
            {
                ...
                if (boxNum == 1)
                {
                    ...
                    txKylinMsg.cbus.cp.x = 0 + kylinOdomCalib.cbus.cp.x;
                    txKylinMsg.cbus.cv.x = XSPEED * ramp;
                    txKylinMsg.cbus.cp.y = 500 + kylinOdomCalib.cbus.cp.y; //3485 + kylinOdomCalib.cbus.cp.y;//3485 + kylinOdomCalib.cbus.cp.y;
                    txKylinMsg.cbus.cv.y = YSPEED * ramp;
                    txKylinMsg.cbus.cp.z = 0;
                    txKylinMsg.cbus.cv.z = 0;
                    txKylinMsg.cbus.gp.e = 0;
